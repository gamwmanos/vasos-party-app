  ...
  return (
    <main className="min-h-screen bg-[var(--color-bg-dark)] p-4 pb-24">
      <header className="flex items-center gap-4 py-4 mb-6">
        ...
        <h2 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#c0392b] to-[#e74c3c] text-glow-cyan">
          After Party Gallery
        </h2>
        {isAdmin && (
      ...
      {/* PARTY OVER BANNER */}
      <div className="mb-8 bg-gradient-to-r from-[#7f1d1d]/60 to-[#1c0a0a]/80 border border-[#c0392b]/40 rounded-2xl p-5 flex items-center gap-4">
        <span className="text-4xl flex-shrink-0">🎉</span>
        <div>
          <p className="text-white font-black text-lg leading-tight">Το πάρτι τελείωσε!</p>
          <p className="text-gray-300 text-sm mt-1">Η συλλογή είναι ανοιχτή για όλους. Δες τις αναμνήσεις της βραδιάς και κατέβασε όσες θες! 📸</p>
        </div>
      </div>

      {items.length === 0 ? (
        <p className="text-gray-400 text-center mt-20">Δεν βρέθηκαν φωτογραφίες.</p>
      ) : (
          ...
          {QUESTS.map((quest) => {
            ...
            return (
              <div key={quest.id} className="space-y-4">
                ...
                {/* PARTY OVER: Everyone sees all photos */}
